...
for i in range(5):
    a = np.asarray(Image.open(ROOT / 'raw' / f'roll_bg_{i}.png').convert('RGB')).astype(np.float32)
    s = Image.open(ROOT / 'raw' / f'seam_bg_{i}.png').convert('RGB').resize((a.shape[1], a.shape[0]), Image.LANCZOS)
    ...
    img.save(out / f'bg_{i}.webp', quality=82, method=6); print(f'bg_{i}', img.size)
